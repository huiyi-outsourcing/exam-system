...
namespace ExamResult.utils {
    public class ExcelHelper {
        ...
        public void SaveToExcel(string excelName, IList<ExamSystem.entities.ExamResult> dataTable) {
            try {
                if (dataTable != null) {
        ...
        private void FillSheet(IList<ExamSystem.entities.ExamResult> results) {
            object[] header = CreateHeader();
            WriteData(header, results);
        }


        private void WriteData(object[] header, IList<ExamSystem.entities.ExamResult> results) {
            object[,] objData = new object[results.Count, header.Length];

            for (int j = 0; j < results.Count; j++) {
                var item = results[j];
                objData[j, 0] = item.User.Name;
                objData[j, 1] = item.Occupation.Description;
                objData[j, 2] = item.Score.ToString();
                objData[j, 3] = item.DateTime.ToString();
            }
            AddExcelRows("A2", results.Count, header.Length, objData);
            AutoFitColumns("A1", results.Count + 1, header.Length);
        }
